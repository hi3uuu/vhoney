...
## 20200630
- **Release vHoney 2.0**
  - After 4 months of public testing with the NextGen edition vHoney 2.0 can finally be released.
- **Debian Buster**
  - With the release of Debian Buster vHoney now has access to all packages required right out of the box.
- **Add new honeypots**
  - [Dicompot](https://github.com/nsmfoo/dicompot) by @nsmfoo is a low interaction honeypot for the Dicom protocol which is the international standard to process medical imaging information. Together with Medpot which supports the HL7 protocol vHoney is now offering a Medical Installation type.
  - [Honeysap](https://github.com/SecureAuthCorp/HoneySAP) by SecureAuthCorp is a low interaction honeypot for the SAP services, in case of vHoney configured for the SAP router.
...
- **Install Types**
  ...
  - There is now a **Medical** Installation Type with Dicompot and Medpot which will be of most interest for medical institutions to get started with vHoney.
- **Update Tools**
  - Connecting to vHoney via `https://<ip>:64297` brings you to the vHoney Landing Page now which is based on Heimdall and the latest NGINX enforcing TLS 1.3.
  - The ELK stack was updated to 7.8.0 and stripped down to the necessary core functions (where possible) for vHoney while keeping ELK RAM requirements to a minimum (8GB of RAM is recommended now). The number of index pattern fields was reduced to **697** which increases performance significantly. There are **22** Kibana Dashboards, **397** Kibana Visualizations and **24** Kibana Searches readily available to cover all your needs to get started and familiar with vHoney.
...
- **Landing Page**
  - After logging into vHoney via web you are now greeted with a beautifully designed landing page.
- **Countless Tweaks and improvements**
  - Under the hood lots of tiny tweaks, improvements and a few bugfixes will increase your overall experience with vHoney.

## 20200316
- **Move from Sid to Stable**
  - Debian Stable has now all the packages and versions we need for vHoney. As a consequence we can now move to the `stable` branch.
...
- **Bump Nextgen to 2.0**
  ...
  - ElasticStack bumped to 7.6.1 (Elasticsearch will need at least 2048MB of RAM now, vHoney at least 8GB of RAM) and tweak to accomodate changes of 7.x.
  ...
  - Base vHoney landing page on Heimdall.
...
- **Upgrades and rebuilds**
  ...
  - Add 1080p vHoney wallpaper for download

## 20190824
- **Add some logo work**
  - Thanks to @thehadilps's suggestion adjusted social preview
  - Added 4k vHoney wallpaper for download
...
- **Bump ELK stack to 6.7.2**
  - Transition to 7.x must iterate slowly through previous versions to prevent changes breaking vHoneys
...
## 20190701
- **Reworked Ansible vHoney Deployment**
...
- **Start supporting Fatt, remove Glastopf**
  ...
  - Glastopf is no longer supported within vHoney
...
## 20190513
- **Added Ansible vHoney Deployment on Open Telekom Cloud**
...
## 20190404
- **Fix #332**
  - If vHoney, opposed to the requirements, does not have full internet access netselect-apt fails to determine the fastest mirror as it needs ICMP and UDP outgoing. Should netselect-apt fail the default mirrors will be used.
