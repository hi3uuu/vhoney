...
vHoney 20.06 runs on Debian (Stable), is based heavily on
...
Furthermore vHoney includes the following tools
...
* [ELK stack](https://www.elastic.co/videos) to beautifully visualize all the events captured by vHoney.
...
# TL;DR
1. Meet the [system requirements](#requirements). The vHoney installation needs at least 8 GB RAM and 128 GB free disk space as well as a working (outgoing non-filtered) internet connection.
2. Download the vHoney ISO from [GitHub](https://github.com/hi3uuu/vhoney/releases) or [create it yourself](#createiso).
...
vHoney is based on the Debian (Stable) network installer.
The honeypot daemons as well as other support components are [dockered](http://docker.io).
This allows vHoney to run multiple honeypot daemons and tools on the same network interface while maintaining a small footprint and constrain each honeypot within its own environment.

In vHoney we combine the dockerized honeypots ...
* [adbhoney](https://github.com/huuck/ADBHoney),
* [ciscoasa](https://github.com/Cymmetria/ciscoasa_honeypot),
* [citrixhoneypot](https://github.com/MalwareTech/CitrixHoneypot),
* [conpot](http://conpot.org/),
...
While data within docker containers is volatile vHoney ensures a default 30 day persistence of all relevant honeypot and tool data in the well known `/data` folder and sub-folders. The persistence configuration may be adjusted in `/opt/vhoney/etc/logrotate/logrotate.conf`. Once a docker container crashes, all other data produced within its environment is erased and a fresh instance is started from the corresponding docker image.<br>
...
The source code and configuration files are fully stored in the vHoney GitHub repository. The docker images are preconfigured for the vHoney environment. If you want to run the docker images separately, make sure you study the docker-compose configuration (`/opt/vhoney/etc/vhoney.yml`) and the vHoney systemd script (`/etc/systemd/system/vhoney.service`), as they provide a good starting point for implementing changes.

The individual docker configurations are located in the [docker folder](https://github.com/hi3uuu/vhoney/tree/master/docker).
...
<a name="types"></a>
# Installation Types
There are prebuilt installation types available each focussing on different aspects to get you started right out of the box. The docker-compose files are located in `/opt/vhoney/etc/compose`. If you want to build your own compose file just create a new one (based on the layout and settings of the prebuilds) in `/opt/vhoney/etc/compose` and run `tped.sh` afterwards to point vHoney to the new compose file and run you personalized edition.
...
# Installation
The installation of vHoney is straight forward and heavily depends on a working, transparent and non-proxied up and running internet connection. Otherwise the installation **will fail!**

Firstly, decide if you want to download the prebuilt installation ISO image from [GitHub](https://github.com/hi3uuu/vhoney/releases), [create it yourself](#createiso) ***or*** [post-install on an existing Debian 10 (Buster)](#postinstall).

Secondly, decide where you the system to run: [real hardware](#hardware) or in a [virtual machine](#vm)?

<a name="prebuilt"></a>
## Prebuilt ISO Image
An installation ISO image is available for download (~50MB), which is created by the [ISO Creator](https://github.com/hi3uuu/vhoney) you can use yourself in order to create your own image. It will basically just save you some time downloading components and creating the ISO image.
You can download the prebuilt installation ISO from [GitHub](https://github.com/hi3uuu/vhoney/releases) and jump to the [installation](#vm) section.

<a name="createiso"></a>
## Create your own ISO Image
For transparency reasons and to give you the ability to customize your install you use the [ISO Creator](https://github.com/hi3uuu/vhoney) that enables you to create your own ISO installation image.
...
```
git clone https://github.com/hi3uuu/vhoney
cd vhoney
```
2. Run the `makeiso.sh` script to build the ISO image.
The script will download and install dependencies necessary to build the image on the invoking machine. It will further download the ubuntu network installer image (~50MB) which vHoney is based on.
```
sudo ./makeiso.sh
```
After a successful build, you will find the ISO image `vhoney.iso` along with a SHA256 checksum `vhoney.sha256` in your folder.

<a name="vm"></a>
## Running in VM
You may want to run vHoney in a virtualized environment. The virtual system configuration depends on your virtualization provider.

vHoney is successfully tested with [VirtualBox](https://www.virtualbox.org) and [VMWare](http://www.vmware.com) with just little modifications to the default machine configurations.
...
If you want to use a wifi card as a primary NIC for vHoney, please be aware that not all network interface drivers support all wireless cards. In VirtualBox e.g. you have to choose the *"MT SERVER"* model of the NIC.

Lastly, mount the `vhoney.iso` ISO to the VM and continue with the installation.<br>

You can now jump [here](#firstrun).

<a name="hardware"></a>
## Running on Hardware
If you decide to run vHoney on dedicated hardware, just follow these steps:

1. Burn a CD from the ISO image or make a bootable USB stick using the image. <br>
Whereas most CD burning tools allow you to burn from ISO images, the procedure to create a bootable USB stick from an ISO image depends on your system. There are various Windows GUI tools available, e.g. [this tip](http://www.ubuntu.com/download/desktop/create-a-usb-stick-on-windows) might help you.<br> On [Linux](http://askubuntu.com/questions/59551/how-to-burn-a-iso-to-a-usb-device) or [MacOS](http://www.ubuntu.com/download/desktop/create-a-usb-stick-on-mac-osx) you can use the tool *dd* or create the USB stick with vHoney's [ISO Creator](https://github.com/hi3uuu).
...
The vHoney Universal Installer will upgrade the system and install all required vHoney dependencies.

Just follow these steps:

```
git clone https://github.com/hi3uuu/vhoney
cd vhoney/iso/installer/
...
## Post-Install Auto
You can also let the installer run automatically if you provide your own `vhoney.conf`. An example is available in `vhoney/iso/installer/vhoney.conf.dist`. This should make things easier in case you want to automate the installation i.e. with **Ansible**.

Just follow these steps while adjusting `vhoney.conf` to your needs:

```
git clone https://github.com/hi3uuu/vhoney
cd vhoney/iso/installer/
cp vhoney.conf.dist vhoney.conf
./install.sh --type=auto --conf=vhoney.conf
```
...
<a name="placement"></a>
# System Placement
Make sure your system is reachable through a network you suspect intruders in / from (i.e. the internet). Otherwise vHoney will most likely not capture any attacks, other than the ones from your internal network! For starters it is recommended to put vHoney in an unfiltered zone, where all TCP and UDP traffic is forwarded to vHoney's network interface. However to avoid fingerprinting you can put vHoney behind a firewall and forward all TCP / UDP traffic in the port range of 1-64000 to vHoney while allowing access to ports > 64000 only from trusted IPs.
...
In case you need external Admin UI access, forward TCP port 64294 to vHoney, see below.
In case you need external SSH access, forward TCP port 64295 to vHoney, see below.
In case you need external Web UI access, forward TCP port 64297 to vHoney, see below.

vHoney requires outgoing git, http, https connections for updates (Debian, Docker, GitHub, PyPi), attack submission (ewsposter, hpfeeds) and CVE / IP reputation translation map updates (logstash, listbot). Ports and availability may vary based on your geographical location. Also during first install outgoing ICMP / TRACEROUTE is required additionally to find the closest and fastest mirror to you.

<a name="updates"></a>
# Updates
For the ones of you who want to live on the bleeding edge of vHoney development we introduced an update feature which will allow you to update all vHoney relevant files to be up to date with the vHoney master branch.
**If you made any relevant changes to the vHoney relevant config files make sure to create a backup first.**

The Update script will:
 - **mercilessly** overwrite local changes to be in sync with the vHoney master branch
 - upgrade the system to the packages available in Debian (Stable)
 - update all resources to be in-sync with the vHoney master branch
 - ensure all vHoney relevant system files will be patched / copied into the original vHoney state
...
## Maintenance
vHoney is designed to be low maintenance. Basically, there is nothing you have to do but let it run.
...
As an Opt-In it is now possible to also share vHoney data with 3rd party HPFEEDS brokers.  
If you want to share your vHoney data you simply have to register an account with a 3rd party broker with its own benefits towards the community. You simply run `hpfeeds_optin.sh` which will ask for your credentials. It will automatically update `/opt/vhoney/etc/vhoney.yml` to deliver events to your desired broker.
...
You are always invited to participate in development on our [GitHub](https://github.com/hi3uuu/vhoney) page.
...
<a name="faq"></a>
# FAQ
Please report any issues or questions on our [GitHub issue list](https://github.com/hi3uuu/vhoney/issues), so the community can participate.

<a name="contact"></a>
# Contact
The software is provided **as is** in a Community Edition format. vHoney is designed to run out of the box and with zero maintenance involved. <br>
We hope you understand that we cannot provide support on an individual basis. We will try to address questions, bugs and problems on our [GitHub issue list](https://github.com/hi3uuu/vhoney/issues).

<a name="licenses"></a>
# Licenses
The software that vHoney is built on uses the following licenses.
...
<br>GPLv3: [adbhoney](https://github.com/huuck/ADBHoney), [elasticpot](https://gitlab.com/bontchev/elasticpot/-/blob/master/LICENSE), [ewsposter](https://github.com/hi3uuu/ews/), [fatt](https://github.com/0x4D31/fatt/blob/master/LICENSE), [rdpy](https://github.com/citronneur/rdpy/blob/master/LICENSE), [heralding](https://github.com/johnnykv/heralding/blob/master/LICENSE.txt), [snare](https://github.com/mushorg/snare/blob/master/LICENSE), [tanner](https://github.com/mushorg/snare/blob/master/LICENSE)
...
<a name="credits"></a>
# Credits
Without open source and the fruitful development community (we are proud to be a part of), vHoney would not have been possible! Our thanks are extended but not limited to the following people and organizations:
...
<a name="staytuned"></a>
# Stay tuned ...
A new version of vHoney is released about every 6-12 months, development has shifted more and more towards rolling releases and the usage of `/opt/vhoney/update.sh`.

<a name="testimonial"></a>
# Testimonial
One of the greatest feedback we have gotten so far is by one of the Conpot developers:<br>
***"[...] I highly recommend vHoney which is ... it's not exactly a swiss army knife .. it's more like a swiss army soldier, equipped with a swiss army knife. Inside a tank. A swiss tank. [...]"***
