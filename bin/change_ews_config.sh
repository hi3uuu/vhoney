...
read apiToken
printf "[*] If you have multiple vHoneys running, give them each a unique NUMBER, e.g. '2' for your second vHoney installation. Enter unique number for THIS vHoney: "
read indexNumber
if ! [[ "$indexNumber" =~ ^[0-9]+$ ]]
    then
...
echo "API URL: " $apiURL
echo "Unique numeric ID for your vHoney Installation: "  $indexNumber
...
wget -q https://raw.githubusercontent.com/hi3uuu/vhoney/master/docker/ews/dist/ews.cfg -O ews.cfg.dist 
...
echo "[+] Changing vhoney.yml to include new ews.cfg."

cp ews.cfg /data/ews/conf/ews.cfg
cp /opt/vhoney/etc/vhoney.yml /opt/vhoney/etc/vhoney.yml.bak
sed -i '/- \/data\/ews\/conf\/ews.ip:\/opt\/ewsposter\/ews.ip/a\ \ \   - \/data\/ews\/conf\/ews.cfg:\/opt\/ewsposter\/ews.cfg' /opt/vhoney/etc/vhoney.yml

echo "[+] Restarting vHoney."
systemctl restart vhoney
echo "[+] Done."
