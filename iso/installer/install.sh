#!/bin/bash
# vHoney Universal Installer

# Installer can only be executed once.
myVHONEY_INSTALL_LOG="/install.log"
if [ -s "$myVHONEY_INSTALL_LOG" ];
  then
...
myVHONEYCOMPOSE="/opt/vhoney/etc/vhoney.yml"
...
myINFO="\
############################################
### vHoney Installer for Debian (Stable) ###
############################################

Disclaimer:
This script will install vHoney on this system.
By running the script you know what you are doing:
1. SSH will be reconfigured to tcp/64444.
...
$myRANDOM_MINUTE $myPULL_HOUR * * *      root    docker-compose -f /opt/vhoney/etc/vhoney.yml pull

# Delete elasticsearch logstash indices older than 90 days
$myRANDOM_MINUTE $myDEL_HOUR * * *      root    curator --config /opt/vhoney/etc/curator/curator.yml /opt/vhoney/etc/curator/actions.yml

# Uploaded binaries are not supposed to be downloaded
*/1 * * * *     root    mv --backup=numbered /data/dionaea/roots/ftp/* /data/dionaea/binaries/

# Daily reboot
$myRANDOM_MINUTE $myRANDOM_HOUR * * 1-6      root    systemctl stop vhoney && docker stop \$(docker ps -aq) || docker rm \$(docker ps -aq) || reboot
...
function fuCHECKNET {
  if [ "$myVHONEY_DEPLOYMENT_TYPE" == "iso" ] || [ "$myVHONEY_DEPLOYMENT_TYPE" == "user" ];
    then
      local mySITES="$1"
...
EOF
  ...
  echo
  echo "### Installing vHoney dependencies."
  echo
...
if [ "$myVHONEY_DEPLOYMENT_TYPE" == "user" ];
  then
    echo
    ...
    echo "### might collide with vHoney's honeypots and prevent vHoney from starting successfully."
    echo
    while [ 1 != 2 ]
      do
...
for i in "$@"
  do
    case $i in
      --conf=*)
        myVHONEY_CONF_FILE="${i#*=}"
        shift
      ;;
      --type=user)
        myVHONEY_DEPLOYMENT_TYPE="${i#*=}"
        shift
      ;;
      --type=auto)
        myVHONEY_DEPLOYMENT_TYPE="${i#*=}"
      ...
      --help)
        ...
        echo
        echo "--conf=<Path to \"vhoney.conf\">"
	echo "  Use this if you want to automatically deploy a vHoney instance (--type=auto implied)."
        echo "  A configuration example is available in \"vhoney/iso/installer/vhoney.conf.dist\"."
        echo
        echo "--type=<[user, auto, iso]>"
	echo "  user, use this if you want to manually install a vHoney on a Debian (Stable) machine."
        echo "  auto, implied if a configuration file is passed as an argument for automatic deployment."
        echo "  iso, use this if you are a vHoney developer and want to install a vHoney from a pre-compiled iso."
        echo
	exit
      ;;
      *)
...
if [ "$myVHONEY_DEPLOYMENT_TYPE" == "auto" ] && [ "$myVHONEY_CONF_FILE" == "" ];
  then
    echo "Aborting. No configuration file given."
    exit
fi
if [ -s "$myVHONEY_CONF_FILE" ] && [ "$myVHONEY_CONF_FILE" != "" ];
  then
    myVHONEY_DEPLOYMENT_TYPE="auto"
    if [ "$(head -n 1 $myVHONEY_CONF_FILE | grep -c "# vhoney")" == "1" ];
      then
        source "$myVHONEY_CONF_FILE"
      else
	echo "Aborting. Config file \"$myVHONEY_CONF_FILE\" not a vHoney configuration file."
        exit
      fi
  elif ! [ -s "$myVHONEY_CONF_FILE" ] && [ "$myVHONEY_CONF_FILE" != "" ];
    then
      echo "Aborting. Config file \"$myVHONEY_CONF_FILE\" not found."
      exit
fi
...
if [ "$myVHONEY_DEPLOYMENT_TYPE" == "iso" ] || [ "$myVHONEY_DEPLOYMENT_TYPE" == "user" ];
  then
    myCONF_VHONEY_FLAVOR=$(dialog --keep-window --no-cancel --backtitle "$myBACKTITLE" --title "[ Choose Your vHoney Edition ]" --menu \
...
if [ "$myVHONEY_DEPLOYMENT_TYPE" == "iso" ];
  then
    ...
    printf "%s" "$myCONF_VHONEY_USER:$myPASS1" | chpasswd
fi
...
if ! [ "$myCONF_VHONEY_FLAVOR" == "SENSOR" ];
  then
...
addgroup --gid 2000 vhoney
adduser --system --no-create-home --uid 2000 --disabled-password --disabled-login --gid 2000 vhoney

# Let's set the hostname
a=$(fuRANDOMWORD /opt/vhoney/host/usr/share/dict/a.txt)
n=$(fuRANDOMWORD /opt/vhoney/host/usr/share/dict/n.txt)
myHOST=$a$n
...
# Let's make sure only myCONF_VHONEY_FLAVOR images will be downloaded and started
case $myCONF_VHONEY_FLAVOR in
  STANDARD)
    fuBANNER "STANDARD"
    ln -s /opt/vhoney/etc/compose/standard.yml $myVHONEYCOMPOSE
  ;;
  SENSOR)
    fuBANNER "SENSOR"
    ln -s /opt/vhoney/etc/compose/sensor.yml $myVHONEYCOMPOSE
  ;;
  INDUSTRIAL)
    fuBANNER "INDUSTRIAL"
    ln -s /opt/vhoney/etc/compose/industrial.yml $myVHONEYCOMPOSE
  ;;
  COLLECTOR)
    fuBANNER "COLLECTOR"
    ln -s /opt/vhoney/etc/compose/collector.yml $myVHONEYCOMPOSE
  ;;
  NEXTGEN)
    fuBANNER "NEXTGEN"
    ln -s /opt/vhoney/etc/compose/nextgen.yml $myVHONEYCOMPOSE
  ;;
  MEDICAL)
    fuBANNER "MEDICAL"
    ln -s /opt/vhoney/etc/compose/medical.yml $myVHONEYCOMPOSE
  ;;
esac
...
echo "$myFAIL2BANCONF" | tee /etc/fail2ban/jail.d/vhoney.conf
...
tar xvfz /opt/vhoney/etc/objects/elkbase.tgz -C /
cp /opt/vhoney/host/etc/systemd/* /etc/systemd/system/
systemctl enable vhoney
...
# Let's enable a color prompt and add /opt/vhoney/bin to path
fuBANNER "Setup prompt"
tee -a /root/.bashrc <<EOF
$mySHELLCHECK
$myROOTPROMPT
$myROOTCOLORS
PATH="$PATH:/opt/vhoney/bin"
EOF
for i in $(ls -d /home/*/)
  do
tee -a $i.bashrc <<EOF
...
# Final steps
cp /opt/vhoney/host/etc/rc.local /etc/rc.local && \
...
if [ "$myVHONEY_DEPLOYMENT_TYPE" == "auto" ];
  then
